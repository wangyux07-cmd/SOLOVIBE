...
export default function App() {
  ...
  const [currentScreen, setCurrentScreen] = useState<ScreenId>("challenge");
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-neutral-900 text-neutral-100 font-sans selection:bg-amber-300 selection:text-neutral-900 antialiased overflow-x-hidden">
      ...
      <div className="flex-1 p-6 md:p-12 lg:max-w-md flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-neutral-800 bg-neutral-950/40 backdrop-blur-3xl z-10">
        <div>
          ...
          <div className="space-y-3 mb-8">
            <h3 className="text-xs font-mono font-bold tracking-widest uppercase text-neutral-400 mb-2">
              快速查验 8 大高保真界面 :
            </h3>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <button
                onClick={() => {
                  setCurrentScreen("challenge");
                  setChallengeTab("task");
                  setPrevScreens([]);
                }}
                className={`py-2 px-3 rounded-lg text-left transition-all flex items-center gap-1.5 ${
                  currentScreen === "challenge" && challengeTab === "task"
              ...
              <button
                ...
                className={`py-2 px-3 rounded-lg text-left transition-all flex items-center gap-1.5 ${
                  currentScreen === "challenge" && challengeTab === "pk"
                    ? "bg-amber-400 text-neutral-950 font-semibold"
                    : "bg-neutral-900 hover:bg-neutral-800 text-neutral-300"
                }`}
              >
      ...
      <div className="flex-1 flex items-center justify-center p-4 bg-neutral-950 select-none relative">
        ...
        >
          ...
          <div className="flex-1 pt-22 pb-20 overflow-y-auto overflow-x-hidden hide-scrollbar scroll-smooth bg-[#fcf9f8]">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* 1. SCREEN: CHALLENGE (今日挑战与PK合并版) */}
                {currentScreen === "challenge" && (
                  <div className="px-5 py-3 space-y-5">
                    
                    {/* Selectors Tag */}
                    <div className="flex justify-between items-center">
                      <span className="px-3 py-1 bg-stone-100 rounded-full font-label-md text-[11px] text-[#4e4632] font-semibold border border-stone-200/50">
                        今日副本
                      </span>
                    ...
                    <div>
                      ...
                      <p className="text-[#4e4632] text-xs font-body-md mt-1">
                        精选一人高质量独行副本，记录专属你的轻量漫步与专属时光 🌿
                      </p>
                    </div>

                    {/* Highly intuitive segmented tab selector */}
                    <div className="flex p-1 bg-stone-100/95 rounded-full border border-stone-200/50 mt-1 shadow-xs select-none">
                      <button
                        onClick={() => setChallengeTab("task")}
                        className={`flex-1 py-1.5 px-3 rounded-full text-xs font-bold transition-all duration-200 flex items-center justify-center gap-1 cursor-pointer ${
                          challengeTab === "task"
                    ...
                    {challengeTab === "task" ? (
                      // SUB-TAB: TASK DETAILS & CHIPS
                      <div className="space-y-5">
                        {/* Sparkle advice block */}
                        ...
                        {/* Active Challenge Card (sitting inside unvisited shop 30 min) */}
                        ...
                        <div className="space-y-3 pt-2">
                          <button 
                            ...
                            className="w-full py-4 rounded-full bg-amber-400 hover:bg-amber-300 font-bold text-neutral-950 text-[15px] shadow-sm cursor-pointer transition-all active:scale-95 duration-150 flex items-center justify-center gap-2"
                          >
                            开启今日漫游 <ArrowRight className="w-4 h-4" />
                          </button>
                          ...
                          <button 
                            ...
                            className="w-full text-center text-xs font-bold text-[#725c00] underline py-2 block cursor-pointer"
                          >
                            查看我的漫游进度 ⚡
                          </button>
                        </div>
                      ...
                      // SUB-TAB: OWN PROGRESS AND STATUS TRACKER (NON-COMPARATIVE)
                      <div className="space-y-5">
                        {/* Info notification */}
                        <div className="p-3 bg-amber-50/60 border border-amber-200/30 rounded-xl flex items-center gap-2 shadow-xs">
                          <HelpCircle className="w-4 h-4 text-amber-600 shrink-0" />
                          <p className="text-[10px] text-[#725c00] leading-tight flex-1">
                            漫游专注进行中。在这里，你不需要面对任何无意义的社交压力和喧嚣，只需静候脚步，享受并感受此时此刻。✨
                          </p>
                        </div>

                        {/* Active Challenge Progress Card */}
                        <div className="p-4 bg-white/80 rounded-2xl border border-stone-200/60 relative shadow-xs shrink-0">
                          <div className="flex justify-between items-center text-[10px] text-stone-500 font-semibold uppercase mb-2">
                            <span className="flex items-center gap-1 bg-stone-100 text-stone-700 px-2 py-0.5 rounded-full font-bold">
                              <Zap className="w-3 h-3 text-amber-600 fill-amber-600 animate-pulse" /> 进行中
                            </span>
                            <span className="flex items-center gap-1 text-[#725c00] font-bold">
                              <Clock className="w-3.5 h-3.5" />
                              已专注 15 分钟 / 共 30 分钟
                            </span>
                          </div>

                          <h3 className="font-extrabold text-stone-900 text-sm leading-normal">
                            {wanderPlans[0].title}
                          </h3>

                          <div className="flex gap-2 mt-2.5 p-0.5 shrink-0">
                            <span className="px-2 py-0.5 rounded text-[9px] font-bold text-stone-500 bg-stone-100">
                              个人探索
                            </span>
                            <span className="px-2 py-0.5 rounded text-[9px] font-bold text-[#00694d] bg-green-50">
                              无需社交
                            </span>
                            <span className="px-2 py-0.5 rounded text-[9px] font-bold text-stone-500 bg-stone-100">
                              低心理压力
                          ...
                          <div className="bg-white/80 border border-stone-200/60 rounded-2xl p-4 shadow-xs">
                            ...
                            <div className="space-y-3 font-medium">
                              <div className="flex items-start gap-2.5">
                                <div className="w-4.5 h-4.5 rounded-full bg-green-100 text-green-700 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                                  ✓
                                </div>
                                <div className="text-left">
                                  <p className="text-xs font-bold text-stone-850">1. 探索方案确认</p>
                                  <p className="text-[10px] text-stone-400">已成功为你预留河畔黄金座位并计入行程表</p>
                                </div>
                              </div>
                              ...
                              <div className="flex items-start gap-2.5">
                                ...
                                <div className="text-left">
                                  <p className="text-xs font-bold text-stone-850">2. 启程至漫游点</p>
                                  <p className="text-[10px] text-stone-400">大步跨出舒适圈，来到 1km 内的漫步目标目的地</p>
                                </div>
                              </div>

                              <div className="flex items-start gap-2.5">
                                <div className="w-4.5 h-4.5 rounded-full bg-amber-100 text-[#725c00] font-bold text-[11px] flex items-center justify-center shrink-0 mt-0.5 animate-pulse">
                                  ⏳
                                </div>
                                <div className="text-left">
                                  <p className="text-xs font-bold text-stone-855">3. 个人静享专注</p>
                                  <p className="text-[10px] text-stone-500">听歌、呼吸或观察，正享受专属你的沉浸 30 分钟</p>
                                </div>
                              </div>

                              <div className="flex items-start gap-2.5">
                                <div className="w-4.5 h-4.5 rounded-full bg-stone-100 text-stone-400 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                                  4
                                </div>
                                <div className="text-left">
                                  <p className="text-xs font-bold text-stone-400">4. 行程终章盖章</p>
                                  <p className="text-[10px] text-stone-400">点击下方确认打卡，解锁并收获今日的旅行勋章</p>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="pt-2 space-y-2">
                          ...
                          <button 
                            ...
                            className="w-full text-center py-2.5 text-xs text-stone-500 font-bold hover:text-stone-800 block cursor-pointer"
                          >
                            返回查看原任务详情
                          </button>
                        </div>
                ...
                {currentScreen === "chat" && (
                  <div className="h-full flex flex-col justify-between">
                    ...
                    <div className="flex-1 px-4 py-3 overflow-y-auto space-y-4 max-h-[480px]">
                    ...
                    <div className="px-4 py-2 border-t border-stone-100 bg-[#fcf9f8]/95 space-y-2">
                      ...
                      <button
                        ...
                        className="w-full bg-[#1c1b1b] text-white text-[11px] font-bold py-3.5 px-4 rounded-xl shadow-md flex justify-center items-center gap-2 active:scale-95 duration-150 transition-all border border-neutral-800"
                      >
                ...
                {currentScreen === "solutions" && (
                  <div className="px-5 py-3 space-y-5">
                    ...
                    <div>
                      ...
                      <p className="text-[#4e4632] text-xs font-body-md mt-1">
                        基于你当下的 <span className="text-[#725c00] font-bold">心情</span>、
                        <span className="text-[#725c00] font-bold">剩余时间</span>及
                        <span className="text-[#725c00] font-bold">今日预算</span>生成。
                      </p>
                    </div>
                    ...
                    <div className="pt-2">
                      <button 
                        onClick={() => {
                          ...
                          setChallengeTab("pk");
                          setCurrentScreen("challenge");
                          alert("🎉 预约成功！Solo 已经帮你预留了河畔咖啡 15:30 黄金靠窗位。漫游单已加入日常日历，同时你已自动开启同城挑战比拼排位！");
                        }}
                        className="w-full py-4 bg-amber-400 hover:bg-amber-300 font-bold text-neutral-900 text-[15px] rounded-full shadow-md flex items-center justify-center gap-2 cursor-pointer transition-transform active:scale-95 duration-100"
                      >
                        <Zap className="w-4 h-4 shrink-0 fill-neutral-900" />
                        确认并一键预约
                      </button>
                ...
                {currentScreen === "map" && (
                  <div className="px-5 py-3 space-y-4">
                    ...
                    <div className="pt-2">
                      ...
                      <button 
                        onClick={() => pushScreen("resonance")}
                        className="w-full text-center py-2 text-xs text-stone-500 font-bold hover:text-stone-800 block cursor-pointer"
                      >
                        查看我的打卡记录
                      </button>
                    </div>
          ...
          {/* LOWER NAVIGATION COHESIVE BAR (首页 Explore, 行程 Map, 消息 Chat, 我的 Profile) */}
          <nav className="absolute bottom-0 left-0 w-full h-18 bg-[#fcf9f8]/95 backdrop-blur-xl border-t border-stone-200/50 flex justify-around items-center pt-2 pb-safe z-(100) shadow-[0_-2px_15px_rgba(0,0,0,0.02)] select-none">
            {/* Nav item: Homely Explorer (Today's Challenge) */}
            <button
              onClick={() => {
                setPrevScreens([]);
                setCurrentScreen("challenge");
              ...
              className={`flex-1 flex flex-col items-center justify-center py-1 transition-all duration-200 cursor-pointer ${
                currentScreen === "challenge" && challengeTab === "task" ? "text-[#725c00] font-bold" : "text-stone-400 hover:text-stone-500"
              }`}
            >
              <Award className={`w-5 h-5 mb-0.5 ${currentScreen === "challenge" && challengeTab === "task" ? "fill-amber-400/20" : ""}`} />
              <span className="text-[10px] tracking-wide">首页</span>
            </button>

            {/* Nav item: Instances/Map tracker */}
            <button
              onClick={() => {
                setPrevScreens([]);
                setCurrentScreen("map");
              ...
              className={`flex-1 flex flex-col items-center justify-center py-1 transition-all duration-200 cursor-pointer ${
                currentScreen === "map" || currentScreen === "index" ? "text-[#725c00] font-bold" : "text-stone-400 hover:text-stone-500"
              }`}
            >
              <MapIcon className="w-5 h-5 mb-0.5" />
              <span className="text-[10px] tracking-wide">行程</span>
            </button>

            {/* Nav item: Chat Companion screen with custom sparkles notification icon */}
            <button
              onClick={() => {
                setPrevScreens([]);
                setCurrentScreen("chat");
              }}
              className={`flex-grow-0 shrink-0 px-4 py-1 flex flex-col items-center justify-center rounded-full transition-all duration-200 cursor-pointer ${
                currentScreen === "chat" || currentScreen === "solutions" || currentScreen === "booking"
                  ? "bg-amber-400 text-neutral-950 font-bold" 
                  : "text-stone-400 hover:text-stone-500"
              }`}
            >
              <MessageSquare className={`w-5 h-5 ${currentScreen === "chat" || currentScreen === "solutions" || currentScreen === "booking" ? "fill-neutral-950/10" : ""}`} />
              <span className="text-[10px] tracking-wide">消息</span>
            </button>

            {/* Nav item: PK Leaderboards / Social resonance tabs */}
            <button
              onClick={() => {
                setPrevScreens([]);
                setCurrentScreen("challenge");
              ...
              className={`flex-1 flex flex-col items-center justify-center py-1 transition-all duration-200 cursor-pointer ${
                currentScreen === "challenge" && challengeTab === "pk" ? "text-[#725c00] font-bold" : "text-stone-400 hover:text-stone-500"
              }`}
            >
              <Activity className="w-5 h-5 mb-0.5" />
              <span className="text-[10px] tracking-wide">挑战</span>
            </button>

            {/* Nav item: Resonance Profile details */}
            <button
              onClick={() => {
                setPrevScreens([]);
                setCurrentScreen("resonance");
